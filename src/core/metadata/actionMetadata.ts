...
import { ActionMetadataArgs } from '@core/metadata/args/action';
import { ActionType } from '@core/types/action';
import { ClassTransformOptions } from 'class-transformer';
import { ControllerMetadata } from '@core/metadata/controllerMetadata';
import { InterceptorMetadata } from '@core/metadata/interceptorMetadata';
import { ParamMetadata } from '@core/metadata/paramMetadata';
import { ResponseHandlerMetadata } from '@core/metadata/responseHandlerMetadata';
...
export class ActionMetadata {
  ...
  static appendBaseRoute(baseRoute: string, route: RegExp | string) {
    const prefix = `${
      baseRoute.length > 0 && baseRoute.indexOf('/') < 0 ? '/' : ''
    }${baseRoute}`;
  ...
  build(responseHandlers: ResponseHandlerMetadata[]) {
    ...
    const undefinedResultHandler = responseHandlers.find(
      (handler) => handler.type === 'on-undefined'
    );
    const nullResultHandler = responseHandlers.find(
      (handler) => handler.type === 'on-null'
    );
    const successCodeHandler = responseHandlers.find(
      (handler) => handler.type === 'success-code'
    );
    const redirectHandler = responseHandlers.find(
      (handler) => handler.type === 'redirect'
    );
    const renderedTemplateHandler = responseHandlers.find(
      (handler) => handler.type === 'rendered-template'
    );
    const authorizedHandler = responseHandlers.find(
      (handler) => handler.type === 'authorized'
    );
    const contentTypeHandler = responseHandlers.find(
      (handler) => handler.type === 'content-type'
    );
    const bodyParam = this.params.find((param) => param.type === 'body');

    if (classTransformerResponseHandler)
      this.responseClassTransformOptions =
        classTransformerResponseHandler.value;

    this.undefinedResultCode = undefinedResultHandler
      ? undefinedResultHandler.value
      : this.globalOptions.defaults &&
        this.globalOptions.defaults.undefinedResultCode;

    this.nullResultCode = nullResultHandler
      ? nullResultHandler.value
      : this.globalOptions.defaults &&
        this.globalOptions.defaults.nullResultCode;
    ...
    this.isBodyUsed = !!this.params.find(
      (param) => param.type === 'body' || param.type === 'body-param'
    );
    ...
    this.isAuthorizedUsed =
      this.controllerMetadata.isAuthorizedUsed || !!authorizedHandler;
    this.authorizedRoles = (
      this.controllerMetadata.authorizedRoles || []
    ).concat((authorizedHandler && authorizedHandler.value) || []);
  }
  ...
  private buildFullRoute(): string | RegExp {
    if (this.route instanceof RegExp) {
      if (this.controllerMetadata.route) {
        return ActionMetadata.appendBaseRoute(
          this.controllerMetadata.route,
          this.route
        );
      }
      return this.route;
  ...
  private buildHeaders(responseHandlers: ResponseHandlerMetadata[]) {
    ...
    const locationHandler = responseHandlers.find(
    ...
    const headerHandlers = responseHandlers.filter(
      (handler) => handler.type === 'header'
    );
    if (headerHandlers)
      headerHandlers.map(
        (handler) => (headers[handler.value] = handler.secondaryValue)
      );
